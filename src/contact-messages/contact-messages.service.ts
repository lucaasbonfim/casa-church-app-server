...
import {
  ...
  UPDATED_CONTACT_MESSAGE,
  DELETED_CONTACT_MESSAGE,
  NOT_FOUND_CONTACT_MESSAGE,
} from "./contact-messages.constants";
...
export class ContactMessagesService {
  ...
  async create(createContactMessageDto: CreateContactMessageDto) {
    ...
    const contactMessage =
      await this.contactMessagesRepository.create(contactMessageData);
    return {
  ...
  async findAll(
    findContactMessagesQuery: FindContactMessagesQueryDto,
  ...
  async update(
    ...
    const updatedContactMessage = await this.contactMessagesRepository.update(
      id,
